...
CalculatorHistory_v2 <- R6Class(
  
  classname = "CalculatorHistory_v2",
  ...
  public = list(
    ...
    process_history = function(
      ...
      self$df <- do.call(rbind, lapply(unique(self$df$engine_id), function(esn){
        ...
        df_esn <- self$process_params_smooth(df_esn,smooth_window)
        
        df_esn <- self$process_params_mean(df_esn,number_of_obs_mean,threshhold)
    ...
    process_params_smooth = function(df_esn,smooth_window = 30){
    ...
    process_params_mean = function(df_esn,number_of_obs_mean=15, threshhold=2,cartoonist=1){
      ...
      do.call(rbind, lapply(unique(df_esn$event_cum), function(event_cum){
        ...
        if(event_cum > 0 & nrow(df_esn_event) > 0){
          ...
          if(nrow(df_esn_event_prev) > 0){
            ...
            else if(cartoonist == -1){
              
              mean_float_value_custom_before_wash <- max(
                tail(
                  df_esn_event_prev$float_value_smooth_custom, 
                  number_of_obs_mean
                )
              )
              mean_float_value_custom_after_wash <- min(
                head(
                  df_esn_event$float_value_smooth_custom, 
                  number_of_obs_mean
                )
            ...
            if(
              nrow(
                df_esn_event[
                  cartoonist*df_esn_event$float_value_smooth_custom <= cartoonist*mean_float_value_custom_before_wash + threshhold,
                  ]) > 0
              ){
